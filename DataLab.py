...
# To replace the white space in headers with underline
train_data.columns = train_data.columns.str.replace(' ', '_')
test_data.columns = test_data.columns.str.replace(' ', '_')
print(train_data.columns)

# Looking for object data to see how to catogorize them
...
cat_rep = {'Zielvariable':{'nein':0,'ja':1}}
train_data.replace(cat_rep, inplace=True)

# change the continues elmiable kontostand into three categories (negative, positive and null)
...
cat_elms_list=['Kontostand','Familienstand','Haus','Kredit','Ausfall_Kredit','Kontaktart',
          'Schulabschluß', 'Ergebnis_letzte_Kampagne','Art_der_Anstellung','Monat']
for elm in cat_elms_list:
    cat_elm_train='elm'+'_'+ elm
    cat_elm_train = pd.get_dummies(train_data[elm], prefix=elm)
    cat_elm_test= 'elm' + '_' + elm
    cat_elm_test = pd.get_dummies(test_data[elm], prefix=elm)
    data1 = train_data.join(cat_elm_train)
    data2 = test_data.join(cat_elm_test)
    train_data = data1
    test_data = data2

data_elms_train = train_data.columns.values.tolist()
data_elms_test = test_data.columns.values.tolist()
keep_train = [i for i in data_elms_train if i not in cat_elms_list]
Train_dt = train_data[keep_train]
keep_test=[i for i in data_elms_test if i not in cat_elms_list]
Test_dt=test_data[keep_test]
print(Train_dt.columns)
print(Test_dt.columns)

# droping some unuseful elmiables
...
print("Length of oversampled data: ",len(smt_data_X))
print("Ratio of zero target value in oversampled data : ",len(smt_data_y[smt_data_y['Zielvariable'] == 0])/len(smt_data_X))
print("Ratio of one target value in oversampled data :  ",len(smt_data_y[smt_data_y['Zielvariable'] == 1])/len(smt_data_X))
...
print('Accuracy of LogisticRegressionClassifier on test set: {:.2f}'.format(logreg.score(X_test,y_test)))
print('LogisticRegression (cross_validation):',metrics.accuracy_score(y, predicted))
...
print('Accuracy of DecisionTreeClassifier on test set: {:.2f}'.format(dtc.score(X_test, y_test)))
...
for i in range(0, len(Z)):
    ...
    cv_results = model_selection.cross_val_score(model, selected_x, y, cv = k_fold, scoring = 'accuracy')
...
fig.suptitle('Comparing the Algorithm')
...
for m in models:
    ...
    fp, tp, thresholds = metrics.roc_curve(y_test, model.predict_proba(X_test)[:, 1])

    auc = metrics.roc_auc_score(y_test, model.predict(X_test))

    plt.plot(fp, tp, label='%s ROC (area = %0.2f)' % (m['label'], auc))
